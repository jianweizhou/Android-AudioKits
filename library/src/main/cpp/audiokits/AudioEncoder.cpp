...
//    testFile = fopen("/mnt/sdcard/xiaokai.aac", "wb+");
    ...
    LOGD("audioChannels is %d", audioChannels);
    ...
    LOGD("avCodecContext->channels is %d", avCodecContext->channels);
    avCodecContext->flags |= CODEC_FLAG_GLOBAL_HEADER;

    /* find the MP3 encoder */

    AVCodec *aac = avcodec_find_encoder_by_name("aac");
    AVCodec *libx264 = avcodec_find_encoder_by_name("libx264");
    AVCodec *libmp3lame = avcodec_find_encoder_by_name("libmp3lame");
    AVCodec *libfdk_aac = avcodec_find_encoder_by_name("libfdk_aac");
    AVCodec *libfdk_aac1 = avcodec_find_encoder_by_name("libfdk-aac");


    codec = avcodec_find_encoder(AV_CODEC_ID_MP3);
//    codec = avcodec_find_encoder_by_name(codec_name);
    if (!codec) {
        LOGD("Couldn't find a valid audio codec");
        return -1;
    }
    ...
    if (preferedChannels != avCodecContext->channels
        ...
        LOGD("channels is {%d, %d}", preferedChannels, audioStream->codec->channels);
        LOGD("sample_rate is {%d, %d}", preferedSampleRate, audioStream->codec->sample_rate);
        LOGD("sample_fmt is {%d, %d}", preferedSampleFMT, audioStream->codec->sample_fmt);
        LOGD("AV_SAMPLE_FMT_S16P is %d AV_SAMPLE_FMT_S16 is %d AV_SAMPLE_FMT_FLTP is %d",
...
int AudioEncoder::alloc_avframe() {
    ...
    samplesCursor = 0;
    if (!samples) {
        LOGD("Could not allocate %d bytes for samples buffer\n", buffer_size);
        return -2;
    }
    LOGD("allocate %d bytes for samples buffer\n", buffer_size);
    ...
    if (ret < 0) {
        LOGD("Could not setup audio frame\n");
    }
    if (swrContext) {
        if (av_sample_fmt_is_planar(avCodecContext->sample_fmt)) {
            LOGD("Codec Context SampleFormat is Planar...");
        }
        /* 分配空间 */
        ...
        LOGD("After av_malloc swrBuffer");
        /* 此时data[0],data[1]分别指向frame_buf数组起始、中间地址 */
        swrFrame = av_frame_alloc();
        if (!swrFrame) {
            LOGD("Could not allocate swrFrame frame\n");
            return -1;
        }
        ...
        ret = avcodec_fill_audio_frame(swrFrame, avCodecContext->channels,
                                       ...
                                       swrBufferSize, 0);
        LOGD("After avcodec_fill_audio_frame");
        if (ret < 0) {
            LOGD("avcodec_fill_audio_frame error ");
            return -1;
        }
    }
    return ret;
...
int AudioEncoder::init(int bitRate, int channels, int sampleRate, int bitsPerSample,
    ...
    LOGD("aacFilePath is %s ", aacFilePath);
    if ((ret = avformat_alloc_output_context2(&avFormatContext, NULL, NULL, aacFilePath)) != 0) {
        LOGD("avFormatContext   alloc   failed : %s", av_err2str(ret));
        return -1;
    }
    ...
    if (ret = avio_open2(&avFormatContext->pb, aacFilePath, AVIO_FLAG_WRITE, NULL, NULL)) {
        LOGD("Could not avio open fail %s", av_err2str(ret));
        return -1;
    }
...
//	LOGD("begin encode packet..................");
    ...
    if (ret < 0) {
        LOGD("Error encoding audio frame\n");
        return;
    }
    if (got_output) {
...
//    }
    ...
    if (this->isWriteHeaderSuccess) {
        ...
        LOGD("duration is %.3f", this->duration);
    ...
    LOGD("end destroy!!! totalEncodeTimeMills is %d totalSWRTimeMills is %d", totalEncodeTimeMills,
         totalSWRTimeMills);
}